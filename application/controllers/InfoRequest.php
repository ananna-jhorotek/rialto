<?php
class InfoRequest extends CI_Controller{
    ...
    public function request(){
		...
		unset($requestDataArray['appointment']); 
		// echo '<pre/>';
		...
		$insert_id = $this->InfoRequest_model->storeLogs($requestDataArray);	
		
			// echo '<pre/>';
			// print_r($insert_id);
			// die;
		if($requestDataArray['msisdn'])
		{
			// $this->InfoRequest_model->getData($requestDataArray['msisdn']);	
			$isDndListed = $this->InfoRequest_model->getData($requestDataArray['msisdn']);	
			
			// var_dump($isDndListed);
			// die;
			if($isDndListed != false)
			{
				$this->InfoRequest_model->updateRequest($insert_id);
			}
