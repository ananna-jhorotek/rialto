<!-- Content Wrapper. Contains page content -->
                        ...
                        <div class="col-sm-12 col-md-12 col-lg-12">
                              <div class="main">
                                    <div class="row">
                                                          ...
                                                          <div class="form-group">
                                                              ...
                                                              <select class="form-control" id="wing_id">
                                                                  ...
                                                                  <?php foreach ($wings as $wing): ?>
                                                                      <option  value="<?= $wing['id']; ?>"><?= $wing['crime_name']; ?></option>
                                                                  <?php endforeach; ?> 
                                                              </select>   
                                                          </div>
                                          ...
                                          <div class="col-lg-6 col-xs-6">
                                                <div class="panel panel-success">
                                                    <div class="panel-heading">Sub Crime List</div>   
                                                    <div class="panel-body"> 
  ...
  <div class="modal fade" id="myModal2" tabindex="-1" style="margin-left:25px; margin-top: 50px;" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                ...
                <form  enctype="multipart/form-data" method="post" action="<?= site_url('settings/edit_sub_crimes');?>">
                  <div class="modal-content">
                        <div class="modal-header">
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                                <h4 class="modal-title">Edit Sub Crime Information</h4>
                        </div>
...
<script>
    $(document).ready(function(){ 
            $.ajax({
                async : false,
                dataType : "html",
                url : "<?= site_url('settings/subcrime_list')?>",
    ...
    $('body').on('click','#save_user_type',function(){ 
        ...
        if (deployment_name == null || deployment_name == " ") {
            ...
            var url = "<?= site_url('settings/save_sub_crime');?>";
            $.ajax({
                    type : "post",
                    ...
                    success : function(result){	
                                $('#user_type_list').html(result);
                                //window.location.replace("<?= site_url('settings/sub_crime'); ?>"); 
                            },
            });
            }
		...
		var url = "<?= site_url('settings/sub_crime_delete');?>";
                    $.ajax({
                            type : "post",
...
<script>
    $(document).ready(function () {
     ...
     $('body').on('click', '#edit_info', function () {          
         ...
         var url = '<?= site_url('settings/ajax_edit_sub_crime_page'); ?>';
            $.ajax({
                    type : "post",
