                        ...
                        <div class="col-sm-12 col-md-12 col-lg-12">
                              <div class="main">
                                    <div class="row">
                                                ...
                                                <div class="panel panel-success">
                                                      <div class="panel-body">    
                                                          <div class="form-group" id="userNameDiv">
                                                              <label for="txtFloor"><span class="errorStar"></span> Wing :</label>
                                          ...
                                          <div class="col-lg-6 col-xs-6">
                                                <div class="panel panel-success">
                                                    <div class="panel-heading">Wing List</div>   
                                                    <div class="panel-body"> 
    ...
    $(document).ready(function () {
        $('body').on('input', '#get_user_type', function () {
           var designation = $('#get_user_type').val();            
            $.ajax({
                async: false,
                url: "<?= site_url('settings/check_existing_wing'); ?>",
                type: "POST",
...
<script>
		$(document).ready(function(){
            $.ajax({
                async : false,
                dataType : "html",
                url : "<?= site_url('settings/wing_list')?>",
                    success : function(result){
				...
				//------------Adding logs to DB------------
					...
					//------------Adding logs to DB------------
					if(user_type_id !=0){
						var url = "<?= site_url('settings/edit_wing');?>";
						$.ajax({
								type : "post",
								...
								success : function(result){	
										        $('#user_type_list').html(result);
											$('#get_user_type').val('');
											window.location.replace("<?= site_url('settings/wing'); ?>"); 
										},
						});
                    }
					else{
						...
						if (user_type == null || user_type == "") {
							...
							var url = "<?= site_url('settings/save_wing');?>";
							$.ajax({
									type : "post",
				...
				$('body').on('click','.delete_user_type',function(){
					...
					//------------Adding logs to DB------------
						...
						var url = "<?= site_url('settings/wing_delete');?>";
						$.ajax({
                            type : "post",
