<!-- Content Wrapper. Contains page content -->
  ...
  <div class="modal fade" id="myModal2" tabindex="-1" style="margin-left:25px; margin-top: 50px;" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                ...
                <form  enctype="multipart/form-data" method="post" action="<?= site_url('settings/edit_appointment'); ?>">
    ...
    $(document).ready(function () {
        $('body').on('input', '#deployment_name', function () {
           var designation = $('#deployment_name').val();  
            ...
            $.ajax({
                async: false,
                url: "<?= site_url('settings/check_existing_unit'); ?>",
                type: "POST",
...
<script>
    $(document).ready(function(){   
            $.ajax({
                async : false,
                dataType : "html",
                url : "<?= site_url('settings/appointment_list')?>",
                    success : function(result){
    ...
    $('body').on('click','#save_user_type',function(){ 
        ...
        if (deployment_name == null || deployment_name == " ") {
            ...
            var url = "<?= site_url('settings/save_appointment');?>";
            $.ajax({
                    type : "post",
                    ...
                    success : function(result){	
                                $('#user_type_list').html(result);
                                window.location.replace("<?= site_url('settings/appointment'); ?>"); 
                            },
            });
            }
		...
		var url = "<?= site_url('settings/appointment_delete');?>";
                    $.ajax({
                            type : "post",
...
<script>
    $(document).ready(function () {
     ...
     $('body').on('click', '#edit_info', function () {          
         ...
         var url = '<?= site_url('settings/ajax_edit_unit_page'); ?>';
            $.ajax({
                    type : "post",
