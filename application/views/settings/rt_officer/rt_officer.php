<!-- Content Wrapper. Contains page content -->
                        ...
                        <div class="col-sm-12 col-md-12 col-lg-12">
                              <div class="main">
                                    <div class="row">
                                          ...
                                          <div class="col-lg-6 col-xs-6">
                                                <div class="panel panel-success">
                                                    <div class="panel-heading">Reporting Officer List</div>   
                                                    <div class="panel-body"> 
  ...
  <div class="modal fade" id="myModal2" tabindex="-1" style="margin-left:25px; margin-top: 50px;" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                ...
                <form  enctype="multipart/form-data" method="post" action="<?= site_url('settings/edit_rt_officer'); ?>">
                  <div class="modal-content">
                        <div class="modal-header">
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                                <h4 class="modal-title">Edit Reporting Officer Information</h4>
                        </div>
    ...
    $(document).ready(function () {
        $('body').on('input', '#deployment_name', function () {
           var designation = $('#deployment_name').val();  
            ...
            $.ajax({
                async: false,
                url: "<?= site_url('settings/check_existing_rt_officer'); ?>",
                type: "POST",
...
<script>
    $(document).ready(function(){   
            $.ajax({
                async : false,
                dataType : "html",
                url : "<?= site_url('settings/rt_officer_list')?>",
                    success : function(result){
    ...
    $('body').on('click','#save_user_type',function(){ 
        ...
        if (deployment_name == null || deployment_name == " ") {
            ...
            var url = "<?= site_url('settings/save_rt_officer');?>";
            $.ajax({
                    type : "post",
                    ...
                    success : function(result){	
                                $('#user_type_list').html(result);
                                window.location.replace("<?= site_url('settings/rt_officer'); ?>"); 
                            },
            });
            }
		...
		var url = "<?= site_url('settings/rt_officer_delete');?>";
                    $.ajax({
                            type : "post",
...
<script>
    $(document).ready(function () {
     ...
     $('body').on('click', '#edit_info', function () {          
         ...
         var url = '<?= site_url('settings/ajax_edit_rt_officer_page'); ?>';
            $.ajax({
                    type : "post",
